...
$sql = "SELECT * FROM `annoce` LIMIT 6  OFFSET $startIndex " ;
...
$annonce = $statement->fetchAll(PDO::FETCH_ASSOC);
  ...
  $sql = 'SELECT * FROM `annoce` LIMIT 6 OFFSET 0' ;
...
$sql= "SELECT * FROM `annoce` WHERE `price` >= $priceMin   AND `price` <= $priceMax  AND `type`  LIKE '$type' ";

$statement = $conn->query($sql);

// fetch all rows
$serchResult = $statement->fetchAll(PDO::FETCH_ASSOC);
        ...
        <div class="row justify-content-center align-items-center">
          <div class="col-12 col-md-9 text-center mt-5">
            <div class="row justify-content-center align-items-center mb-5">
                ...
                <input type="submit" name="valueSearch" class="btn btn-ajouter col" value="Search">
              </div>
            </div>
          </div>
        </div>
...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
  ...
  foreach ($annonce as $key => $value) {
    ...
    <div class="card mt-3 mx-auto" style="width: 20rem;">
      ...
      <div class="card-body">
        ...
        <p>
          <a class="text-dark" href="detail.php?pageId= <?php echo $value['id']; ?>" > more details...</a>
        </p>
        <a 
  ...
  foreach ($serchResult as $key => $value) {

    ?>
    <div class="card mt-3 mx-auto" style="width: 20rem;">
      <div class="img-zoom">
        <img src="./images/hero_bg_1.jpg" class="card-img-top" alt="image" style="overflow: hidden;">
      </div>
      <div class="card-body">
        ...
        <div class="card-city"><span><?php echo $value['titre']; ?></span></div>
        <p>
        <a href=" detail.php?pageId= <?php echo $value['id']; ?>" > more details...</a>
        </p>
...
<div class="modal fade" id="deleteAnnonce" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">cancel</button>
        <a type="button"  id="deleteBtn" href="" class="btn btn-primary">Delete</a>
      </div>
    </div>
  </div>
</div>
      ...
      <div class="modal-body">
        <form action="Factory.php" method="post">
          ...
          <div class="modal-footer">
            ...
            <button type="submit" class="btn btn-primary">Add a rows</button>
          </div>
        </form>
      </div>
    </div>
